...
public class ClockProtocol {
    ...
    public void listen() {
        try {
            this.channel = this.bootstrap.bind(InetAddress.getLocalHost(), (this.server ? 22565 : 0)).await().channel();
        } catch (UnknownHostException ex) {
            throw new IllegalStateException("Cannot listen on localhost: " + ex.getMessage(), ex);
        } catch (InterruptedException ex) {
            throw new IllegalStateException("Interrupted during channel creation: " + ex.getMessage(), ex);
        }
    }
    ...
    public void push(@Nonnull ClockMessage message) {
        try {
            this.channel.writeAndFlush(new DatagramPacket(this.channel.alloc().buffer().writeBytes(this.writer.writeValueAsBytes(message)), new InetSocketAddress(InetAddress.getLocalHost(), 22565)));
        } catch (JsonProcessingException | UnknownHostException ex) {
            throw new IllegalStateException("Cannot send message from localhost to localhost: " + ex.getMessage(), ex);
        }
    }
    ...
    private class ClientHandler extends SimpleChannelInboundHandler<DatagramPacket> {
         ...
         */
        @Override
        protected void messageReceived(@Nonnull ChannelHandlerContext ctx, @Nonnull DatagramPacket msg) throws Exception {
