...
public class MinecraftAgent {
    ...
    try {
      ...
      try {
        ...
        instrumentation.redefineClasses(
            new ClassDefinition(Class.forName("net.minecraft.client.ClientBrandRetriever"),
                type.toBytecode()));
        type.detach();
      }

      // 1.10 & 1.9.4 & 1.9 & 1.8.9 & 1.8
      if (matches("bcx", "z") || matches("bcd", "z") || matches("bcf", "z") || matches("ave", "A")
          || matches("bsu", "z")) {
        final String name;
        String tickName = "t";
        ...
        instrumentation
            .redefineClasses(new ClassDefinition(Class.forName(name), type.toBytecode()));
        type.detach();
      }
      // 1.7.10
        ...
        instrumentation
            .redefineClasses(new ClassDefinition(Class.forName("bao"), type.toBytecode()));
        type.detach();
      }
  ...
  /**
   * Checks whether the supplied type name and method name match the requirements to be a reference
   * to the Minecraft main class.
   *
   ...
   */
  public static void pushUpdate(@Nonnegative long worldTime, @Nonnegative long worldAge,
      boolean raining) {
